	...
	for _, c := range cases {
		t.Run(c.description, func(t *testing.T) {
			...
			if c.isRemaining != isRemaining {
				t.Errorf("\nUnexpected resultRow: \n\t\t expected: %t, \n\t\t actual: %t", c.isRemaining, isRemaining)
...
func TestFinder(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.description, func(t *testing.T) {
			initTestPlayers()
			result := finder(c.board, c.isMyTurn, 2, ComputerPlayer{"x"})
...
func TestPlay(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.description, func(t *testing.T) {
			initTestPlayers()
			expectedRow, expectedCol := play(c.board,ComputerPlayer{"x"})
...
func initTestPlayers(){
	SetPlayers(ManualPlayer{"o"})
	SetPlayers(ManualPlayer{"z"})
	SetPlayers(ComputerPlayer{"x"})
}